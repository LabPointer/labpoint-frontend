...
export function ReserveForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            try {
                const res = await fetch(
                    `/api/reserve/create/${encodeURIComponent(spaceName)}`,
                    {
                        method: "POST",
    ...
    const reservesQuery = useQuery({
        ...
        queryFn: async () => {
            if (!spaceName) return [];
            const res = await fetch(
                `/api/reserves/${encodeURIComponent(spaceName)}`,
            );
                ...
                >
                                    ...
                                    <Popover>
                                        <PopoverTrigger
                                            render={
                                                <Button
                                                    variant="outline"
                                                    className="w-full justify-start text-left font-normal border border-black/20"
                                                />
                                            }
                                        >
                                            {field.state.value ? (
                    ...
                    <form.Subscribe
                        ...
                        children={([canSubmit, isSubmitting, values]) => {
                            const { date, startTime, endTime } = values as {
                                date: Date | null;
                                startTime: string;
                                endTime: string;
                            };
                            ...
                            const filteredAndSortedReserves = (
                                reservesQuery.data || []
                            )
                                .filter(
                                    (r) =>
                                ...
                                .sort((a, b) => {
                                    ...
                                    return (
                                        a.rStart.getTime() - b.rStart.getTime()
                                    );
                                });
                                ...
                                <>
                                    <div className="pt-4 border-t border-black/10">
                                        ...
                                        <ScrollArea className="h-40 w-full rounded-lg border border-black/15 shadow-md bg-muted/30">
                                            <div className="p-3">
                                                ...
                                                ) : filteredAndSortedReserves.length >
                                                  0 ? (
                                                    <div className="space-y-2">
                                                        {filteredAndSortedReserves.map(
                                                            (res) => (
                                                                <div
                                                                    key={res.id}
                                                                    className={`flex flex-col gap-1.5 p-3 rounded-lg border shadow-xs transition-colors hover:bg-muted/50 ${res.isConflict ? "bg-red-50 border-red-300 border-l-4 border-l-red-600" : "bg-background border-black/10 border-l-4 border-l-indigo-600"}`}
                                                                >
                                                                    <div className="flex items-center justify-between">
                                                                        <div className="flex items-center gap-2">
                                                                            <CalendarIcon
                                                                                className={`w-3.5 h-3.5 ${res.isConflict ? "text-red-600" : "text-primary"}`}
                                                                            />
                                    ...
                                    <div className="pt-4 border-black/20">
                                        <div className="flex flex-row w-full gap-3">
                                            <Button
                                                variant="destructive"
                                                className="flex-1 cursor-pointer border border-red-600/20 shadow-inner"
                                                type="button"
                                                onClick={() =>
                                                    onOpenChange(false)
                                                }

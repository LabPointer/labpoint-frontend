...
import SpaceSearchbar, {
    type SpaceSearchFilters,
} from "@components/SpaceSearchbar";
...
async function getSpaces(filters?: SpaceSearchFilters) {
    ...
    if (!res.ok || result.status === "error") {
        throw new Error(
            result.message || `Erro HTTP ${res.status} ao realizar a busca`,
        );
    }
...
function App() {
    ...
    const [, setQueryFilters, debouncedQueryFilters] =
        useDebounce<SpaceSearchFilters>(
            {
                query: "",
                resources: [],
                minCapacity: 20,
    ...
    const onReserve = useCallback(
        (name: string, capacity: number, resources: string[]) => {
    ...
    return (
        <>
            ...
            <section className="flex items-center justify-center ">
                <SpaceSearchbar onSearch={onSearch} />
            </section>
                        ...
                        {spacesQuery.isFetching && !spacesQuery.data ? (
                            <div className="col-span-full flex flex-col items-center justify-center py-20 gap-4 text-muted-foreground">
                                ...
                                <p className="text-sm font-medium">
                                    Buscando laboratórios disponíveis...
                                </p>
                            </div>
                        ) : spacesQuery.error ? (
